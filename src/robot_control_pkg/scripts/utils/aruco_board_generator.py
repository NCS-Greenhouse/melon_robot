...
PAGE_DPI = 200
MARKER_SIZE = 450 # marker size in pixel
...
def generate_aruco_board(id_0, id_1):
    ...
    # Add marker size in mm
    pixel_size_mm = 210/ board_image.shape[0] # Define the physical size in millimeters # A4: 210*297   
    text_size = cv2.getTextSize(f"Marker Size: {int(pixel_size_mm * MARKER_SIZE)} mm", font, font_scale, font_thickness)[0]
    text_position = (text_size[1], board_image.shape[0] - text_size[1]*3)
    cv2.putText(board_image, f"Marker Size: {int(pixel_size_mm * MARKER_SIZE)} mm", text_position, font, 1, (0, 0, 0), font_thickness, cv2.LINE_AA)
    ...
    # Add the center of coordination
    cv2.circle(board_image,(board_image.shape[1]//2, board_image.shape[0]//2),radius=3,color=(0,0,0),thickness=3)
